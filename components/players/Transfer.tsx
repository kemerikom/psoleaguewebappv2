        ...
        <div className="flex flex-row w-full items-center justify-center space-x-2 hover:bg-blue-300 rounded transition-all">
            <div className="flex flex-1 justify-end">
                {data.from?.teamid!="free"&&
                    <Link href={`/teams/${data.from?.teamid}`} className="flex link transition-all">{data.from?.teamname}</Link>
                }
                {data.from?.teamid=="free"&&
            ...
            <div className="flex flex-1 justify-start">
                {data.to?.teamid!="free"&&
                    <Link href={`/teams/${data.to?.teamid}`} className="flex link transition-all">{data.to?.teamname}</Link>
                }
                {data.to?.teamid=="free"&&
                    <label className="flex">Free</label>
                }
